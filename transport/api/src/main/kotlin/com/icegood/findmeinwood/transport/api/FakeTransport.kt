...
class FakeTransport(override val id: TransportId) : Transport {
    ...
    /** Test helper: inject raw wire bytes (used for signed/management frame tests). */
    suspend fun deliverWire(wire: ByteArray) {
        if (com.icegood.findmeinwood.core.crypto.MgmtCodec.isMgmt(wire)) {
            events.emit(TransportEvent.MgmtFrameReceived(wire, id))
        } else {
            events.emit(TransportEvent.FrameReceived(com.icegood.findmeinwood.core.crypto.FrameCodec.decode(wire), id))
        }
    }
}
