...
class IdentityTest {
    ...
    fun `sign verify roundtrip and reject`() {
        ...
        assertTrue(Identity.verify(pair.public, data, sig))
        assertFalse(Identity.verify(pair.public, data + 1, sig))
        val other = Identity.generate()
        assertFalse(Identity.verify(other.public, data, sig))
        // member ids are stable, 32 bytes and independent of the key algorithm
        assertEquals(memberId.toList(), Identity.memberIdOf(pair).toList())
    }
}
